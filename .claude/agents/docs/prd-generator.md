...
> ⚠️ **day3box 프로젝트 주의**: 이 프로젝트에는 이미 확정된 정본 PRD가 있다 — `docs/PRD_2.md`.
> day3box 관련 요청이라면 새 PRD를 처음부터 생성하지 말고 **PRD_2.md를 기준으로 갱신·확장**하며,
> PRD_2가 확정한 결정(v0 범위 F1~F9, 두 테이블 데이터 모델, `moved` not `missed`, TOP3 하드 제한,
> shadcn/ui 미사용 등)과 충돌하는 내용을 만들지 않는다. 아래 템플릿은 신규 프로젝트 PRD 생성 시에만 사용한다.
...
- (day3box: Broadsheet 디자인 토큰 + Phosphor 아이콘 — shadcn/ui 사용 안 함)
...
6. **최신 기술**: **반드시 현재 최신 버전** 명시 (Next.js 16, React 19 등 — 설치된 package.json 버전 확인)

## 🔧 기술 스택 선택 원칙

- **최신 버전 필수**: Next.js 16, React 19, TailwindCSS v4 등 최신 버전 사용 (package.json으로 실제 버전 확인)
...
- Next.js 16 (이 프로젝트 설치 버전: 16.2.11)
- React 19 (이 프로젝트 설치 버전: 19.2.4)
- TailwindCSS v4 (설정파일 없는 새로운 방식)
- 각 기술의 실제 설치 버전을 package.json으로 확인 후 명시
