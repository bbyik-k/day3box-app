...
- 프로젝트의 CLAUDE.md/AGENTS.md 파일에 명시된 코딩 표준을 준수합니다

**day3box 프로젝트 컨텍스트** (리뷰 기준의 정본):

- 타임박싱 플래너. 정본 문서: `docs/PRD_2.md`(범위·데이터 모델·UX 파라미터), `ROADMAP.md`(현재 Task와 수락 기준)
- 스택: Next.js **16.2.11** (App Router, breaking changes — 불확실하면 `node_modules/next/dist/docs/` 대조), React 19, TypeScript strict, Tailwind v4 + Broadsheet 토큰(shadcn/ui 미사용), Supabase(RLS)
- 확정 규칙: 시간은 int(분), status는 TS union(`'planned'|'done'|'partial'|'moved'`, `missed` 금지), `as`/`any` 금지, 상태관리 React 내장만, 드래그는 pointer 직접 구현(dnd 라이브러리 금지), 겹침 금지·15분 스냅·TOP3 하드 제한
...
2. **검토 항목**:
   ...
   - **프로젝트 표준**: TypeScript 타입 안전성, Next.js 16 베스트 프랙티스, TailwindCSS v4 규칙
...
4. **특별 고려사항 (day3box)**:
   - Next.js 16 App Router 패턴 준수 확인 (async request APIs — `params`/`searchParams`/`cookies`/`headers` await 처리)
   - TypeScript 타입 안전성 검증 (`as`/`any` 사용 시 심각도 높음으로 지적)
   - React Server Components vs Client Components 경계 적절성 (`'use client'` 최소화)
   - Broadsheet 디자인 토큰 준수 (세리프 타이포, 상태 색 규칙 — `moved`에 빨강/실패 색 금지)
   - 시간 계산이 int(분) 산술로 일관되는지, 겹침 검사·15분 스냅 로직의 엣지 케이스
   - Supabase 클라이언트 사용 규칙 (server/client 구분, 전역 선언 금지) 및 RLS 우회 여부
   - **v0 범위(PRD F1~F9) 밖 기능·미래 대비 코드가 섞였는지** (오버엔지니어링 방어 — 발견 시 지적)
   - 한국어 주석 및 문서화 규칙 준수
